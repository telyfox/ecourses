...
var E3 = {
    ...
    // 格式化支付类型
	formatPaymentType : function formatPaymentType(val,row){
        if (val == 1){
            return '在线支付';
        } else if(val == 2){
    ...
    // 格式化支付状态
	formatPaymentStatus : function formatPaymentStatus(val,row){
        if (val == 1){
    ...
    // 格式化评价状态
	formatBuyerRate : function formatBuyerRate(val,row){
        if (val == 1){
            return '<span style="color:black;">已评价</span>';
        }else {
