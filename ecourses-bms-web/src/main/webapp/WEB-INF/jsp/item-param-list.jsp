...
<table class="easyui-datagrid" id="itemParamList" title="课程列表" 
    ...
    <thead>
        <tr>
        	...
        	<th data-options="field:'itemCatId',width:80">课程类目ID</th>
        	<th data-options="field:'itemCatName',width:100">课程类目</th>
...
<script>
    ...
    var itemParamListToolbar = [{
        ...
        handler:function(){
        	...
        	if(ids.length == 0){
        		$.messager.alert('提示','未选中课程规格!');
        		return ;
        	}
        	$.messager.confirm('确认','确定删除ID为 '+ids+' 的课程规格吗？',function(r){
        	    if (r){
        	    	var params = {"ids":ids};
                	$.post("/item/param/delete",params, function(data){
            			if(data.status == 200){
            				$.messager.alert('提示','删除课程规格成功!',undefined,function(){
