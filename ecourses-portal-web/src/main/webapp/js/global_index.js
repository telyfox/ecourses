...
function timeFun(){
	...
	if(nowtimes >= 1414944000000 && nowtimes<=1415203199000){
		...
		$(".q_ticket a").attr('href','http://www.ecourses.cn/html/activity/1414059663.html');
		$(".q_ticket").show();
		...
		$(".q_ticket a").attr('href','http://www.ecourses.cn/html/activity/1414059592.html');
		$(".q_ticket").show();
...
function isOnline(home_url,passport_url){
	...
	$.getJSON( "/ajax/isOnline/?callback=?", function( data ) {
		if (data.welcome){
			...
			var welComeMsg = '';
			if('嘿' == nickName){
				welComeMsg = nickName+'，欢迎来易课寄在线购课系统！';
			}else{
